package com.example.salfino.naviblind_110217;

/**
 * Created by Saviour on 16/07/2017.
 */

public class TextEntry {

    private String introtext;
    private String locationtext;
    private String menutext;
    private String roomtext;
    private String waypointtext;
    private String confirmationtext;
    private String repeattext;
    private String servicetext;
    private String eventsintrotext;
    private String noeventstext;
    private String routetext;


    public String getIntrotext() {
        return introtext;
    }

    public void setIntrotext(String introtext) {
        this.introtext = introtext;
    }

    public String getLocationtext() {
        return locationtext;
    }

    public void setLocationtext(String locationtext) {
        this.locationtext = locationtext;
    }

    public String getMenutext() {
        return menutext;
    }

    public void setMenutext(String menutext) {
        this.menutext = menutext;
    }

    public String getRoomtext() {
        return roomtext;
    }

    public void setRoomtext(String roomtext) {
        this.roomtext = roomtext;
    }

    public String getWaypointtext() {
        return waypointtext;
    }

    public void setWaypointtext(String waypointtext) {
        this.waypointtext = waypointtext;
    }

    public String getConfirmationtext() {
        return confirmationtext;
    }

    public void setConfirmationtext(String confirmationtext) {
        this.confirmationtext = confirmationtext;
    }

    public String getRepeattext() {
        return repeattext;
    }

    public void setRepeattext(String repeattext) {
        this.repeattext = repeattext;
    }

    public String getServicetext() {
        return servicetext;
    }

    public void setServicetext(String servicetext) {
        this.servicetext = servicetext;
    }

    public String getEventsintrotext() {
        return eventsintrotext;
    }

    public void setEventsintrotext(String eventsintrotext) {
        this.eventsintrotext = eventsintrotext;
    }

    public String getNoeventstext() {
        return noeventstext;
    }

    public void setNoeventstext(String noeventstext) {
        this.noeventstext = noeventstext;
    }

    public String getRoutetext() {
        return routetext;
    }

    public void setRoutetext(String routetext) {
        this.routetext = routetext;
    }

    @Override
    public String toString() {
        return "TextEntry{" +
                "introtext='" + introtext + '\'' +
                ", locationtext='" + locationtext + '\'' +
                ", menutext='" + menutext + '\'' +
                ", roomtext='" + roomtext + '\'' +
                ", waypointtext='" + waypointtext + '\'' +
                ", confirmationtext='" + confirmationtext + '\'' +
                ", repeattext='" + repeattext + '\'' +
                ", servicetext='" + servicetext + '\'' +
                ", eventsintrotext='" + eventsintrotext + '\'' +
                ", noeventstext='" + noeventstext + '\'' +
                ", routetext='" + routetext + '\'' +
                '}';
    }
}
